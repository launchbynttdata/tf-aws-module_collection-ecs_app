...
variable "private_subnets" {
  description = <<EOT
    List of private subnets. ECS services provisioned in private subnets would need NAT gateway to access internet.
...
variable "alb_sg" {
  description = <<EOT
    Security Group for the ALB. The ingress rules specified determine what ports on ALB are open to clients
...
variable "load_balancer_type" {
  description = "The type of the load balancer. Default is 'application'. Valid values are 'application' and 'network'"
  type        = string
  default     = "application"
...
variable "is_internal" {
  description = "Whether this load balancer is internal or public facing. If is_internal=false, then var.public_subnets must be specified"
  type        = bool
  default     = true
}

...
resource "terraform_data" "ecr_push" {
  provisioner "local-exec" {
    command = <<-EOT
      # Use the named profile locally when provided, otherwise rely on ambient AWS credentials.
      aws ecr get-login-password --region ${var.region} ${length(trimspace(var.aws_profile)) > 0 ? "--profile ${var.aws_profile}" : ""} | docker login --username AWS --password-stdin ${data.aws_caller_identity.default.account_id}.dkr.ecr.${var.region}.amazonaws.com
